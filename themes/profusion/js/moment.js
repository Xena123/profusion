...
jQuery(window).load( function() {
    ...
    if(window.location.hash){
        ...
        jQuery('.filter__list li a' + hash + '').trigger('click');
        jQuery('#allcheck').prop('checked', false).trigger('refresh').click();
        jQuery(hash).prop('checked', true).trigger('refresh').click();   
        
         // jQuery('input ' + hash + '').click();
         // jQuery('input#allcheck').click();
    }
});
jQuery(document).ready(function($) {

    jQuery('#gocomment').click(function(e){
        e.preventDefault();
        var section2 = jQuery('#comments').offset().top;                
        jQuery('html, body').animate({scrollTop:section2}, 1000);  
    }); 

    if(window.location.hash){
        jQuery('.comment-respond').slideToggle();
    }
    jQuery('#openrating').click(function(){
        jQuery('.rating-form').slideToggle();
    });

    jQuery('.add-comment, .comment-reply-link').click(function(e){
        e.preventDefault();
        jQuery('.comment-respond').slideToggle();
    });

    function update() {
      jQuery('#time-1').html(moment().tz("Europe/London").format('H:mm a'));
      jQuery('#time-2').html(moment().tz("Asia/Dubai").format('H:mm a'));
    }
    setInterval(update, 1000);

    jQuery('.cut_content').dotdotdot();
    jQuery('.cut_content_2').dotdotdot();

    jQuery('.filter__list li:not(:nth-child(2))').click(function(){
        jQuery('.filter__area__items').addClass('filtered');
    })
    jQuery('.filter__list li:nth-child(2)').click(function(){
        jQuery('.filter__area__items').removeClass('filtered');
    });

    jQuery('.filter__list__sub li:not(:nth-child(2))').click(function(){
        jQuery('.filter__area__items').addClass('filtered');
    });
    jQuery('.filter__list__sub li:nth-child(2)').click(function(){
        jQuery('.filter__area__items').removeClass('filtered');
    });

    jQuery('.blog__area-filter-list li:not(:nth-child(1))').click(function(){
        jQuery('.blog-area-row').addClass('filtered');
    });
    jQuery('.blog__area-filter-list li:nth-child(1)').click(function(){
        jQuery('.blog-area-row').removeClass('filtered');
    });
    // jQuery('.blog-search').focus(function(){
    //     jQuery('.blog-area-row').addClass('filtered');

    // });
    jQuery('.blog-search').keyup(function(){
        jQuery('.blog-area-row').addClass('filtered');
    });
    jQuery('.blog-search').blur(function(){
        if(jQuery('.blog__area-filter-list li:nth-child(1) a').hasClass('is-checked')){
           if( !jQuery(this).val() ){
                //alert('empty');
                jQuery('.blog-area-row').removeClass('filtered'); 
            }
    ...
    jQuery('.partnersgal').owlCarousel({        
        autoplay: true,
        loop: true,
